...
| File | Purpose |
|---|---|
| `main.tf` | Module block with `source = "../../"` (a relative path, not a pinned `?ref` - "baseline" and "PR" are just two on-disk checkouts of this repo), the `azurerm`/`azapi` provider config, and an empty `backend "local" {}` block (path supplied at `init` time - see below). |
| `test_dependencies.tf` | A dedicated, throwaway resource group + vnet (`APP` subnet delegated to `Microsoft.App/environments`, `RZ` subnet for the auto-created registry's private endpoint) this harness owns outright - never a shared/production resource. Names are suffixed with `var.pr_number` so concurrently open PRs never collide. |
| `variables.tf` | `env`, `group`, `project`, `location` (defaults to `canadacentral`), `tags`, `pr_number` (defaults to `"manual"`), and `container-app-environment`/`container-app` (both typed `any`, passed straight through to the module). |
| `config/container-apps.tfvars` | One representative real-usage fixture: a Dedicated ("D4") workload profile, an auto-created container registry + private endpoint, and a single public-image (`nginx:latest`) app with external ingress. |
...
cd test/live
terraform init
terraform plan  -var-file=config/container-apps.tfvars
terraform apply -var-file=config/container-apps.tfvars
```

Confirm only the live-test resource group, vnet/subnets, and
`module.container_apps` are planned/applied, then tear it down:

```bash
terraform destroy -var-file=config/container-apps.tfvars
```
...
terraform apply -var-file=config/container-apps.tfvars -var="pr_number=<pr-number>"

# 2. PR plan (and, in CI, apply), from A, against the same state file.
cd A/test/live
terraform init -backend-config="path=$STATE"
terraform plan -var-file=config/container-apps.tfvars -var="pr_number=<pr-number>"

# 3. Always tear down from A once the run finishes (`if: always()` in CI).
terraform destroy -var-file=config/container-apps.tfvars -var="pr_number=<pr-number>"
```
